...
const AnalyzerWorklet_link = './app_assets/dist/analyzernode.min.js';
    ...
    return new Promise((resolve, reject)=>{
        ...
        try
        {
            ...
            FileAudioContext.audioWorklet.addModule(AnalyzerWorklet_link).then(function() {
                ...
                {
                    if((Date.now() - running.last_node_ms) < 1000)
                    {
                ...
                try
                {
                    FileAudioContext.decodeAudioData(source_binary, function (buffer)
                    {
                        ...
                        FileAudioContext.startRendering().then(function(renderedBuffer) {
                            ...
                            if((running.frames_analyzed >= running.frames_ack) || (running.frames_ack))
                            {
...
async function animate() 
{
    ...
    else if ((plot_config.process_level==3) || (plot_config.process_level>=4))  //segments or formants
    {
        ...
        if((plot_config.last_seg_len != seg_count_now) || (plot_labels_change))       //check if new segment is added by segmentor module
        {
            ...
            for(let si=seg_count_now-1; si>=0; si--)
            {
                ...
                if(seg_size > 0)
                {
                    ...
                    if(plot_config.process_level>=4)
                    {
                        ...
                        if((plot_config.process_level==6) || (plot_config.process_level==7))
                        {
                            plotter.plot_phoneme_anchors(si, segmentor.get_phonemes_ci(si), left_indent)
                        }
                    }
