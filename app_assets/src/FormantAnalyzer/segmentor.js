...
const phn_mod = require('./phonemes.js');
...
const MAX_SEGS = 100; //limit for segments in buffer
...
export function reset_segmentation(process_level, spec_bands, plot_len, window_step, pause_length, min_segment_length, callback=null, test_play=true, file_label=[])
{
    spec_hist = null;
    segments_raw = null;
    segments_fm = null;
    ...
    return new Promise((resolve, reject)=>{
        if(spec_bands)
        {
            ...
            //console.log('reset_segmentation ' + String(spec_bands));
            resolve("ready");
        }
...
function segment_cutter(current_ci)    //called from async peaks_n_valleys_process
{
    ...
    if((seg_size > s_set.seg_min_frames) && (s_set.c_started>=2))
    {
        ...
        //console.log(start_ci + '\t' + seg_size + '\t' + s_set.current_frame);
        
        //level 4 is raw formants, 5 is formant features, 6 is phonemes, 7 is phoneme features
        if(s_set.process_level==7)
        {
            let fm_simple = fm_mod.straighten_formants(this_seg_fm, seg_size, local_minimum);
            //let bin_probs = fm_mod.formant_features(fm_simple[0], seg_size);
            let phn_buff = phn_mod.sep_phonemes(fm_simple[0], fm_simple[1], seg_size);
            let phn_features = phn_mod.phonemes_features(phn_buff[1]);
            ...
            phn_buff[1] = null; //keep the timings, null the phones
            phonemes.push(phn_buff);
            phonemes_ft.push(phn_features);
            resolve(1);
        }
        else if(s_set.process_level==6)
        {
            let fm_simple = fm_mod.straighten_formants(this_seg_fm, seg_size, local_minimum);
            //let bin_probs = fm_mod.formant_features(fm_simple[0], seg_size);
            let phn_buff = phn_mod.sep_phonemes(fm_simple[0], fm_simple[1], seg_size);
        ...
        else if(s_set.process_level==5)
        {
            let fm_simple = fm_mod.straighten_formants(this_seg_fm, seg_size, local_minimum);
            let bin_probs = fm_mod.formant_features(fm_simple[0]);
            segments_labels.push(s_set.current_label.slice());
            segments_ci.push([start_ci, seg_size]);
            segments_raw.push(this_seg_fm);
            segments_fm.push(fm_simple[0]);
            formants_ft.push(bin_probs);
            resolve(1);
        }
        else if(s_set.process_level==4)
        {
            let fm_simple = fm_mod.straighten_formants(this_seg_fm, seg_size, local_minimum);
            segments_labels.push(s_set.current_label.slice());
            segments_ci.push([start_ci, seg_size]);
...
function handle_callback()
{
    ...
    //phonemes are a smaller unit so it sends phoneme every few milliseconds, segments are larger so expect one every half second
    ...
    {
        if(s_set.process_level == 7)    //phonemes features
        {
            while(s_set.callbacks_processed < phonemes_ft.length)
            {
                s_set.callbacks_processed++;
                let cl = s_set.callbacks_processed-1;
                
                if(phonemes_ft[cl].length > 0)
                {
                    if(callback_func) callback_func(cl, s_set.current_label, get_phn_timestamps(cl), phonemes_ft[cl]);
                    else console.warn('callback_func is not set')
                }
            }
            
        }
        else if(s_set.process_level == 6)    //phonemes
        {
            while(s_set.callbacks_processed < phonemes.length)
            {
                s_set.callbacks_processed++;
                let cl = s_set.callbacks_processed-1;
                
                if(phonemes[cl][1].length > 0)
                {
                    if(callback_func) callback_func(cl, s_set.current_label, get_phn_timestamps(cl), phonemes[cl][1]);
                    else console.warn('callback_func is not set')
                }
            }
            
        }
        else if(s_set.process_level == 5)    //formants formants_ft
        {
        ...
        else if(s_set.process_level == 4)    //formant level, same size as segments, but sorted by frequency
        {
        ...
        else if(s_set.process_level == 3)    //segment level
        {
...
export function get_phonemes_ci(segment_index)
{
    return phonemes[segment_index][0];
}

export function get_phn_timestamps(segment_index)  //start time and duration
{
    
   //console.log(phonemes[segment_index][0]);
    const phn_len = phonemes[segment_index][0].length;
    let ph_ts = [];
    try
    {
        for(let ph = 0; ph < phn_len; ph++ ) 
        {
            ph_ts[ph] = [];
            ph_ts[ph][0] = ((segments_ci[segment_index][0] + phonemes[segment_index][0][ph][0]) * s_set.window_step).toFixed(3);
            ph_ts[ph][1] = ((phonemes[segment_index][0][ph][1] + 1) * s_set.window_step).toFixed(3);
        }
    }
...
{
    if((type>=6) && (s_set.process_level>=6)) return segments_fm.length;    //return phonemes
