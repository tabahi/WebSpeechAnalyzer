...
export async function plot_phoneme_anchors(si, phoneme_ci, left_indent)
{
    if(!SpecCtx) return false;

    //console.log(phoneme_ci);
    SpecCtx.font = "10px verdana";
    SpecCtx.textAlign = "left";
    SpecCtx.fillStyle = '#ffffff';
    
    
    for(let ph = 0; ph < phoneme_ci.length; ph++ ) 
    {
        let ci_start = phoneme_ci[ph][0];
        let ci_dist = phoneme_ci[ph][1]+1;
        //console.log(si + '\t' +ci_start +'\t' + ci_dist);
        SpecCtx.fillText(si+","+ph, PIXEL_WIDTH*(left_indent + ci_start), SPEC_BOX_HEIGHT - 6);
